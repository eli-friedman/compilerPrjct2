...
tree temptreeptr = NULL;

int symbol_table_index = 0;
int nestLevel = 0;
int current_nest_level = 0;


// in this function the method nodes will be traversed
// and marked(been edited)
void methodNodeTrav(tree nd2){

	//correct the nesting levels so that the 
	current_nest_level++;
	nestLevel = current_nest_level;
	
	printf("Inside Method Node Traversal\n");
	OpenBlock();
	
	//recurse over the left and right children of the method op
...
//	printf("inside the tree traverser\n");
	...
	if( NodeKind(nd2) == EXPRNode ){
		...
		if(NodeOp(nd2) == MethodOp){
			...
			//this is required so we dont traverse of the method 2x
			return 0;
		}
	}
	...
	if(NodeKind(RightChild(nd2)) == 200){
		...
		//check to see if the value is or isn't already in the string table
		if(GetAttr(symbol_table_index, NEST_ATTR)){
			printf("The value is already in the string table\n");
		}
