 ...
 */
 ...
 int id;
 {
  ...
  for (i = stack_top; i > 0; i--)
    ...
    foundInWholeST = LookUpWholeST (id);
    if (foundInWholeST != -1)
      return foundInWholeST;
...
int loc_str(char* stringName)
{
  ...
  for (i = 0; i < 500; i ++)
  {
    ...
    if (stringTable[i] != NULL)
    {
        ...
        //printf ("%s\n", stringTable[i]);
      if (strcmp (stringName, stringTable[i] ) == 0)
      {
          //printf ("testttt!!!\n");
          //printf ("%s\n", stringTable[i]);
...
//       break;
//   } 
//   return( stringTable[j] );/*return string table indexed at i*/
// }
